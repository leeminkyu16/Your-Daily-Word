...
public class QuotesManager implements IQuotesManager {
	...
	/**
	 * This is a list of runnables that will be executed after the quotes are updated.
	 * This does not include updates to the last modified time.
	 */
	...
	public QuotesManager(IDesktopIoModel ioModel) {
		...
		File quoteFile = new File(ioModel.getLocalApplicationDataDirectory(DEFAULT_FILE_PATH));
		...
		this.afterQuotesUpdateRunnables.add(() -> {
			this.internalModel.lastModified = System.currentTimeMillis();
		});
	}
	...
	public void saveCurrentModelToFile() {
		try (
			FileOutputStream fileOutputStream =
				ioModel.getLocalApplicationDataDirectoryFileOutputStream(DEFAULT_FILE_PATH)
		) {
	...
	public static String DEFAULT_FILE_PATH = "default.ydw";
}
